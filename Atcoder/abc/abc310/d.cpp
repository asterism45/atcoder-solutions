...
bool check(ll idx) {
    for (ll i = 0; i < M; i++) {
        if ((bad[i].first == idx && belong[bad[i].second] == belong[idx]) ||
            (bad[i].second == idx && belong[bad[i].first] == belong[idx])) {
            return false;
        }
    }
    return true;
...
void dfs(ll idx, ll used) {
    ...
    for (ll i = 0; i < min(used + 1, T); i++) {
        belong[idx] = i;
        if (check(idx)) {
            dfs(idx + 1, used + (i == used));
        }
    }
}
