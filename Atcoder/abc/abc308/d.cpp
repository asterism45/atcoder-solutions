...
int main()
{
    ...
    while (!que.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            int next_y = yx.first.first + dy[i], next_x = yx.first.second + dx[i];
            if (next_y < H && next_y > -1 && next_x < W && next_x > -1)
            {
                if ((cu == 's' && S[next_y][next_x] != 'n') || (cu == 'n' && S[next_y][next_x] != 'u') || (cu == 'u' && S[next_y][next_x] != 'k') || (cu == 'k' && S[next_y][next_x] != 'e') || (cu == 'e' && S[next_y][next_x] != 's') || dist[next_y][next_x] != -1)
                    continue;
                dist[next_y][next_x] = dist[yx.first.first][yx.first.second] + 1;
                que.push(mpa(mpa(next_y, next_x), S[next_y][next_x]));
            }
        }
    }
