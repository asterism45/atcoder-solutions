...
ll DFS(ll node, ll par, vll& visited, vll& parent, vvll& G, vll& C) {
    print(C);
    visited[node] = true;
    parent[node] = par;
    ...
    rep(i, G[node].size()) {
        ll child = G[node][i];
        if (C[child] == C[node] || child == par) continue;
        print(node, child);
        print("ok");

        if (!visited[child]) {
            C[node] = 1 - C[node];
            if (DFS(child, node, visited, parent, G, C)) return 1;
            C[node] = 1 - C[node];
        }
        else {
            // ll current = node;
...
int main()
{
    ...
    vll parent(N + 1);
    // C[1] = !C[1];
    Yes(DFS(1, -1, visited, parent, G, C));
    return 0;
}